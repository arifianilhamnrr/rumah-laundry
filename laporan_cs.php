...
// Ambil data order sesuai bulan
$query = "
    SELECT * FROM tb_order_cs 
    WHERE MONTH(tgl_masuk_cs) = '$bulanNum' 
    AND YEAR(tgl_masuk_cs) = '$tahun'
    ORDER BY tgl_masuk_cs ASC
";
...
<title>Laporan Bulanan Cuci Satuan</title>
<style>
...
<h2>🧺 Laporan Bulanan Order Cuci Satuan</h2>
...
<table>
    ...
    <tbody>
        ...
        <tr>
            <td><?= $no++; ?></td>
            <td><?= $d['or_cs_number']; ?></td>
            <td><?= $d['nama_pel_cs']; ?></td>
            <td><?= $d['jenis_paket_cs']; ?></td>
            <td><?= $d['tgl_masuk_cs']; ?></td>
            <td><?= $d['tgl_keluar_cs']; ?></td>
            <td><?= $d['jml_pcs']; ?></td>
