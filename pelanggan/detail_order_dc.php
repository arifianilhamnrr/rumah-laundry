...
if($data['nama_pel_dc'] != $nama_pelanggan){
	echo "<script>
...
<section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
	...
	<div class="mb-8 flex flex-col gap-4 rounded-[28px] border border-slate-200/80 dark:border-slate-800 bg-white/85 dark:bg-slate-900/75 px-6 py-6 shadow-card backdrop-blur sm:flex-row sm:items-center sm:justify-between lg:px-8">
		<div>
			...
			<p class="text-sm text-slate-600 dark:text-slate-400">No. Order: <?= $data['or_dc_number']?></p>
		</div>
	...
	<div class="overflow-hidden rounded-[28px] border border-slate-200/80 dark:border-slate-800 bg-white/90 dark:bg-slate-900/80 shadow-card">
		...
		<div class="p-6 border-b border-slate-200 dark:border-slate-800">
			...
			<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
				<div>
					...
					<p class="text-sm font-medium text-slate-900 dark:text-white"><?= $data['nama_pel_dc'] ?></p>
				</div>
				<div>
					<p class="text-xs text-slate-500 dark:text-slate-400 mb-1">Nomor Telepon</p>
					<p class="text-sm font-medium text-slate-900 dark:text-white"><?= $data['no_telp_dc'] ?></p>
				</div>
				<div class="md:col-span-2">
					<p class="text-xs text-slate-500 dark:text-slate-400 mb-1">Alamat</p>
					<p class="text-sm font-medium text-slate-900 dark:text-white"><?= $data['alamat_dc'] ?></p>
				</div>
			</div>
		</div>
		...
		<div class="p-6 border-b border-slate-200 dark:border-slate-800">
			...
			<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
				<div>
					...
					<p class="text-sm font-medium text-slate-900 dark:text-white"><?= date('d/m/Y', strtotime($data['tgl_masuk_dc'])) ?></p>
				</div>
				<div>
					<p class="text-xs text-slate-500 dark:text-slate-400 mb-1">Tanggal Keluar</p>
					<p class="text-sm font-medium text-slate-900 dark:text-white"><?= date('d/m/Y', strtotime($data['tgl_keluar_dc'])) ?></p>
				</div>
				<div>
					<p class="text-xs text-slate-500 dark:text-slate-400 mb-1">Durasi Kerja</p>
					<p class="text-sm font-medium text-slate-900 dark:text-white"><?= $data['wkt_krj_dc'] ?></p>
				</div>
				<div>
					<p class="text-xs text-slate-500 dark:text-slate-400 mb-1">Jenis Paket</p>
					<p class="text-sm font-medium text-slate-900 dark:text-white"><?= $data['jenis_paket_dc'] ?></p>
				</div>
				<div>
			...
			<div class="rounded-[24px] bg-slate-50 p-4 dark:bg-slate-950/40">
				<div class="grid grid-cols-3 gap-4 text-center">
					<div>
						...
						<p class="text-lg font-bold text-slate-900 dark:text-white"><?= $data['berat_qty_dc'] ?> Kg</p>
					</div>
					<div>
		...
		<div class="p-6">
			...
			<p class="text-sm text-slate-600 dark:text-slate-400"><?= $data['keterangan_dc'] ?></p>
		</div>
	</div>
</section>
