...
function url($url = null){
	...
	// Deteksi base path - ambil hanya folder root project
	$script_name = $_SERVER['SCRIPT_NAME'];
	$script_name = str_replace('\\', '/', $script_name);

	// Cari folder root (rumah_laundry)
	if (preg_match('#(/rumah_laundry)#', $script_name, $matches)) {
		$base_path = $matches[1];
	} else {
		// Fallback: ambil folder pertama setelah root
		$parts = explode('/', trim($script_name, '/'));
		$base_path = '/' . $parts[0];
	}

	// Buat URL utama
	$url_utama = $protocol . $host . $base_path;
...
function order_ck($order_ck){
	...
	$metode = htmlspecialchars($order_ck['metode_pengambilan']); // TAMBAHAN BARU
...
function order_dc($order_dc){
	...
	$metode = htmlspecialchars($order_dc['metode_pengambilan']); // TAMBAHAN BARU
...
function order_cs($order_cs){
	...
	$metode = htmlspecialchars($order_cs['metode_pengambilan']); // TAMBAHAN BARU
...
function register_pelanggan($data){
	...
	// Insert data
	$query = "INSERT INTO pelanggan (nama_lengkap, email, no_telp, alamat, username, password) 
			  VALUES ('$nama_lengkap', '$email', '$no_telp', '$alamat', '$username', '$password_hash')";
...
// Get Riwayat Order Pelanggan (Updated)
function get_order_pelanggan($nama){
	global $koneksi;
	
	$orders = [];
	
	// Order Cuci Komplit
	$ck = mysqli_query($koneksi, "SELECT *, 'Cuci Komplit' as tipe FROM tb_order_ck WHERE nama_pel_ck='$nama' ORDER BY id_order_ck DESC");
	...
	// Order Dry Clean
	$dc = mysqli_query($koneksi, "SELECT *, 'Dry Clean' as tipe FROM tb_order_dc WHERE nama_pel_dc='$nama' ORDER BY id_order_dc DESC");
	while($row = mysqli_fetch_assoc($dc)){
		if(!isset($row['status']) || empty($row['status'])){
			$row['status'] = 'Pending';
		}
		$orders[] = $row;
	...
	// Order Cuci Satuan
	$cs = mysqli_query($koneksi, "SELECT *, 'Cuci Satuan' as tipe FROM tb_order_cs WHERE nama_pel_cs='$nama' ORDER BY id_order_cs DESC");
