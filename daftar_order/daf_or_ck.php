<div class="bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm">
	...
	<div class="px-6 py-4 border-b border-slate-200 dark:border-slate-700">
		<h2 class="text-lg font-semibold text-slate-900 dark:text-white flex items-center">
			<i class="fas fa-soap text-blue-600 dark:text-blue-400 mr-2"></i>
			Order Cuci Komplit
		</h2>
	</div>
	...
	<div class="p-6">
		...
		<div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 mb-4">
			...
			<form method="GET" class="flex gap-2 items-center flex-1">
				<input type="text" name="cari_ck" placeholder="Cari order..."
					value="<?= isset($_GET['cari_ck']) ? $_GET['cari_ck'] : ''; ?>"
					class="flex-1 min-w-[200px] px-3 py-2 text-sm border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400">
				<button type="submit" class="px-4 py-2 text-sm bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-medium transition-colors">
			...
			<!-- Print Form -->
			<form action="laporan_ck.php" method="GET" target="_blank" class="flex gap-2 items-center">
				...
				<tbody class="divide-y divide-slate-200 dark:divide-slate-700">
					...
					if (!empty($cuci_komplit)) :
						...
						<tr class="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
							<td class="px-4 py-3 text-slate-900 dark:text-slate-100"><?= $no_ck; ?></td>
							<td class="px-4 py-3 font-medium text-slate-900 dark:text-slate-100"><?= $ck['or_ck_number'] ?></td>
							<td class="px-4 py-3 text-slate-600 dark:text-slate-400"><?= $ck['tgl_masuk_ck'] ?></td>
							<td class="px-4 py-3 text-slate-600 dark:text-slate-400"><?= $ck['nama_pel_ck'] ?></td>
							<td class="px-4 py-3 text-slate-600 dark:text-slate-400"><?= $ck['jenis_paket_ck'] ?></td>
							<td class="px-4 py-3 text-slate-600 dark:text-slate-400"><?= $ck['wkt_krj_ck'] ?></td>
							<td class="px-4 py-3 text-slate-600 dark:text-slate-400"><?= $ck['berat_qty_ck'] . ' Kg' ?></td>
							...
							<td class="px-4 py-3">
								...
								if($status == 'Pending') {
									...
									$badge_class = 'bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400';
							...
							<td class="px-4 py-3">
								<div class="flex flex-col space-y-2">
									<div class="flex space-x-1">
										<a href="<?=url('detail_order/detail_ck/detail_order_ck.php?or_ck_number=')?><?=$ck['or_ck_number']?>" class="inline-flex items-center px-2 py-1 bg-blue-600 hover:bg-blue-700 text-white rounded text-xs font-medium transition-colors">
											<i class="fas fa-eye text-xs"></i>
										</a>
										<a href="<?=url('daftar_order/hapus_ck.php?or_ck_number=')?><?=$ck['or_ck_number']?>" onclick="return confirm('Yakin akan menghapus?');" class="inline-flex items-center px-2 py-1 bg-red-600 hover:bg-red-700 text-white rounded text-xs font-medium transition-colors">
											<i class="fas fa-trash text-xs"></i>
										</a>
									</div>
									<select onchange="updateStatus(this.value, '<?=$ck['or_ck_number']?>', 'ck')" class="px-2 py-1 text-xs border border-slate-300 dark:border-slate-600 rounded bg-white dark:bg-slate-700 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500">
