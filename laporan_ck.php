...
// Aktifkan error reporting kalau ingin debug
...
// Ambil data orderan sesuai bulan
$query = "
    SELECT * FROM tb_order_ck 
    WHERE MONTH(tgl_masuk_ck) = '$bulanNum' 
    AND YEAR(tgl_masuk_ck) = '$tahun'
    ORDER BY tgl_masuk_ck ASC
";
...
<title>Laporan Bulanan Cuci Komplit</title>
<style>
    body {
...
<body>
    ...
    <h2>🧺 Laporan Bulanan Order Cuci Komplit</h2>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                <td><?= $no++; ?></td>
                <td><?= $d['or_ck_number']; ?></td>
                <td><?= $d['nama_pel_ck']; ?></td>
                <td><?= $d['jenis_paket_ck']; ?></td>
                <td><?= $d['tgl_masuk_ck']; ?></td>
                <td><?= $d['tgl_keluar_ck']; ?></td>
                <td><?= $d['berat_qty_ck']; ?></td>
